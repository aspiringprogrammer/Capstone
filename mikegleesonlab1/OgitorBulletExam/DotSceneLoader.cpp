...
void DotSceneLoader::parseDotScene(const Ogre::String &SceneName, const Ogre::String &groupName, Ogre::SceneManager *yourSceneMgr, Ogre::SceneNode *pAttachNode, const Ogre::String &sPrependNode, btDynamicsWorld *phyWorld)
{
	m_phyWorld = phyWorld;
...
void DotSceneLoader::processTerrain(rapidxml::xml_node<>* XMLNode)
{
	...
	mGroundBody -> getWorldTransform().setOrigin(BtOgre::Convert::toBullet(position) + btVector3(0, 120, 0)); // adjust this number to suit your terrain height
...
void DotSceneLoader::processEntity(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
	...
	btQuaternion bulletOrientation = BtOgre::Convert::toBullet(pParent->getOrientation());
	btDefaultMotionState* groundState = new btDefaultMotionState(
	btTransform(bulletOrientation,bulletPosition));
