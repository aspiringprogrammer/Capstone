 ...
 */
	...
	void VertexIndexToShape::setScale(Ogre::Vector3 set)
	{
		mScale = set;
	}
    ...
    btCapsuleShape* VertexIndexToShape::createCapsule()
    {
            ...
            const float circum = std::max(sz.x,sz.z);
            assert((sz.x > 0.0f) && (sz.y > 0.0f) && (sz.y > 0.0f) && 
                    ("Size of box must be greater than zero on all axes"));
            btCapsuleShape* shape = new btCapsuleShape(circum / 2.0f , sz.y - circum);
	...
	//------------------------------------------------------------------------------------------------
	btCylinderShape* VertexIndexToShape::createCylinder(int x, int y, int z)
	{
		const Ogre::Vector3 sz = getSize();
		...
		//Changed btCylinderShapeX to btCylinderShape (so that it is now in the Y axis)
		if(x > 0)
			shape = new btCylinderShape(Convert::toBullet(sz * 0.5));
		else if(y > 0)
			shape = new btCylinderShapeX(Convert::toBullet(sz * 0.5));
		else if(z > 0)
			shape = new btCylinderShapeZ(Convert::toBullet(sz * 0.5));
